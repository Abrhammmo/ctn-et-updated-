...
export default function Admin({
  ...
  const [expandedPartnerDescriptions, setExpandedPartnerDescriptions] =
  ...
  const submitNews = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newsForm.photos.length)
      return alert("Please upload at least one photo for news.");
    try {
  ...
  const submitEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!eventForm.photos.length)
      return alert("Please upload at least one photo for event.");
    try {
  ...
  const submitTeamMember = async (e: React.FormEvent) => {
    ...
    if (!teamForm.photo_url)
      return alert("Please upload one portrait photo for this team member.");
    try {
  ...
  const deleteTeamMember = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(data.error || "Failed to delete CTNET member");
      await fetchData();
      onTeamMembersChanged();
    } catch (error: any) {
  ...
  const addAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    const invitedEmail = adminForm.email.trim().toLowerCase();
    if (!invitedEmail) {
      alert("Admin email is required");
      return;
    }

    setAdminCreationNotice("");
    ...
    try {
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok)
        throw new Error(data.error || "Failed to generate admin account");

      const returnedPassword =
        typeof data.generatedPassword === "string"
          ? data.generatedPassword
          : "";
      const defaultNotice = `Admin account created. Login credentials were sent to ${invitedEmail}.`;
  ...
  const handlePeopleTabClick = () => {
    setActiveTab("people");
    setPeopleSubTab("contact_notifications");
    setIsSidebarOpen(false);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <div
        className={`w-64 min-w-[16rem] max-w-[16rem] shrink-0 h-screen fixed inset-y-0 left-0 z-40 bg-primary text-white flex flex-col transform transition-transform duration-300 md:sticky md:top-0 md:z-20 md:translate-x-0 ${
      ...
      >
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <button
            ...
            className="text-xl font-bold flex items-center gap-2 text-left"
          >
        ...
        <nav className="flex-grow p-4 space-y-2">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "news" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Newspaper size={20} />
            News
          </button>
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "events" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Calendar size={20} />
            Events
          </button>
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "partners" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Handshake size={20} />
            Partners
          </button>
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "team" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Users size={20} />
            Add CTNET Member
          </button>
          <button
            type="button"
            onClick={handlePeopleTabClick}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl relative ${activeTab === "people" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <Mail size={20} />
            Contacts
            {notificationCount > 0 && (
              <span className="absolute right-4 top-3 min-w-5 h-5 px-1 bg-secondary text-white text-[10px] flex items-center justify-center rounded-full">
                {notificationCount}
              </span>
            )}
          </button>
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "resources" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <BookOpen size={20} />
            Resources
          </button>
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl ${activeTab === "admins" ? "bg-white/20" : "hover:bg-white/10"}`}
          >
            <UserPlus size={20} />
            Manage Admins
          </button>
      ...
      <div className="flex-grow w-full overflow-y-auto p-4 sm:p-6 lg:p-8 md:ml-0">
        <header className="mb-6 sm:mb-8 flex flex-wrap justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setIsSidebarOpen((prev) => !prev)}
              className="md:hidden inline-flex items-center gap-2 px-3 py-2 rounded-xl border border-slate-200 bg-white text-slate-700"
            >
              {isSidebarOpen ? <X size={18} /> : <Menu size={18} />}
              Admin Panel
            </button>
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">
              {tabHeadingLabels[activeTab]} Management
            </h2>
          </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              ...
              >
                <input
                  ...
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, title_en: e.target.value })
                  }
                ...
                <input
                  required
                  value={newsForm.title_am}
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, title_am: e.target.value })
                  }
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, summary_en: e.target.value })
                  }
                ...
                <textarea
                  required
                  value={newsForm.summary_am}
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, summary_am: e.target.value })
                  }
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, description_en: e.target.value })
                  }
                  placeholder="Description (English)"
                  className="md:col-span-2 w-full px-4 py-3 rounded-xl border border-slate-200 h-24"
                />
                <textarea
                  value={newsForm.description_am}
                  onChange={(e) =>
                    setNewsForm({ ...newsForm, description_am: e.target.value })
                  }
          ...
          <div className="space-y-8">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              ...
              >
                ...
                <input
                  required
                  value={eventForm.title_am}
                  onChange={(e) =>
                    setEventForm({ ...eventForm, title_am: e.target.value })
                  }
                  placeholder="Title (Amharic)"
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 font-amharic"
                />
                <textarea
                  ...
                  onChange={(e) =>
                    setEventForm({ ...eventForm, summary_en: e.target.value })
                  }
                  placeholder="Summary (English)"
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 h-24"
                />
                <textarea
                  required
                  value={eventForm.summary_am}
                  onChange={(e) =>
                    setEventForm({ ...eventForm, summary_am: e.target.value })
                  }
                  placeholder="Summary (Amharic)"
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 h-24 font-amharic"
                />
                ...
                />
                <textarea
                  value={eventForm.description_am}
                  onChange={(e) =>
                    setEventForm({
                      ...eventForm,
                      description_am: e.target.value,
                  ...
                  {eventForm.photos.length < 4 && (
                    <label className="w-24 h-24 rounded-xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center text-slate-400 cursor-pointer">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={(e) => handlePhotoUpload(e, "event")}
                      />
                    </label>
                  )}
                </div>
                <button
                  type="submit"
                  className="md:col-span-2 bg-secondary text-white py-4 rounded-2xl font-bold"
                >
                  Publish Event
                </button>
              </form>
              ...
              {partners.map((p) => (
                <div key={p.id} className="bg-white p-4 rounded-2xl border">
                  ...
                  {p.description && (
                    <div className="mt-3">
                      <p
                        className={`text-sm text-slate-600 ${!expandedPartnerDescriptions[p.id] && p.description.length > 140 ? "line-clamp-3" : ""}`}
                      >
                        {p.description}
                      </p>
                      {p.description.length > 140 && (
                        ...
                        >
                          {expandedPartnerDescriptions[p.id]
                            ? "Show less"
                            : "Read more"}
                        </button>
                      )}
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTeamForm({ ...teamForm, member_title: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTeamForm({ ...teamForm, position_role: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTeamForm({ ...teamForm, facebook_url: e.target.value })
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTeamForm({ ...teamForm, x_url: e.target.value })
                  }
                ...
                {contacts.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {contacts.map((c) => (
                      <div
                        key={c.id}
                        className={`bg-white p-6 rounded-2xl border ${c.is_read ? "border-slate-100" : "border-primary/30 bg-primary/5"}`}
                      >
            ...
            {peopleSubTab === "partner_applications" && (
              <div>
                {partnerApplications.length === 0 ? (
                  <p className="text-slate-500">No partner applications yet.</p>
                ) : (
                  <div className="space-y-4">
                    {partnerApplications.map((p) => (
                      <div
                        key={p.id}
                        className={`bg-white p-6 rounded-2xl border ${p.is_read ? "border-slate-100" : "border-secondary/30 bg-secondary/5"}`}
                      >
                        <div className="flex justify-between mb-2">
                          <div>
                            <p className="font-bold">{p.organization}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                          ...
                          <p className="md:col-span-2">
                            <span className="font-bold">Other Category:</span>{" "}
                            {p.other_category || "-"}
                          </p>
                        </div>
                      </div>
                ...
                >
                  ...
                  {resource.resource_type === "online_courses" &&
                    resource.website_link && (
                      <a
                        href={
                          /^https?:\/\//i.test(resource.website_link)
                            ? resource.website_link
                            : `https://${resource.website_link}`
                        }
                        target="_blank"
                        rel="noreferrer"
        ...
        {activeTab === "admins" && (
          <div className="max-w-2xl mx-auto">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-8">
              ...
              {(adminCreationNotice || generatedPassword) && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="mt-8 p-6 bg-green-50 border border-green-100 rounded-2xl"
                >
                  ...
                  {generatedPassword ? (
                    <div className="bg-white p-4 rounded-xl border border-green-200 font-mono text-center text-lg font-bold text-slate-900">
                      {generatedPassword}
                    </div>
                  ) : (
                    <p className="text-xs text-green-700">
                      The email includes the temporary password and asks the new
                      admin to change the default password after first login.
                    </p>
                  )}
                </motion.div>
              )}
                    ...
                    >
                      ...
                      <div className="text-right flex flex-col items-end gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 text-sm font-semibold"
                        >
                          Delete
                        </button>
                      </div>
            ...
            <div className="bg-amber-50 border border-amber-100 p-6 rounded-2xl flex items-start gap-4">
              ...
              <p className="text-amber-700 text-xs mt-1">
                New administrators will be required to change their password
                immediately on first successful login.
              </p>
            </div>
          </div>
        )}
